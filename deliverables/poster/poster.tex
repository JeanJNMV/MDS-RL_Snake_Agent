...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.35\linewidth]{img/Long snake problem.png}
...
\begin{itemize}
  \item \textbf{Frame stacking ($n\!=\!4$):} stacks the last 4 grid observations into a $(4C\!\times\!H\!\times\!W)$ tensor; allows the agent to \emph{infer motion} (e.g.\ obstacle velocity), partially mitigating partial observability and making the state closer to Markovian
  \item \textbf{Dueling + Double DQN:} shared convolutional trunk splits into a state-value head $V(s)$ and an advantage head $A(s,a)$; $Q(s,a) = V(s) + \big(A(s,a) - \bar{A}(s)\big)$; improves stability (Double DQN) and learning efficiency when many actions have similar value (Dueling)
\end{itemize}
\textbf{Reward engineering}
\begin{itemize}
  \item \textbf{Distance shaping:} $r_t \mathrel{+}= \lambda\,(d_{t-1} - d_t)$ toward nearest gold/silver; provides dense signal when food is scarce on cluttered maps
  \item \textbf{Body-proximity shaping:} $r_t \mathrel{+}= \mu\,(b_t - b_{t-1})$ where $b_t$ is distance to nearest body segment; discourages spiral trapping as snake grows
  \item \textbf{Step penalty ($-0.01$/step):} penalises stalling; forces the agent to make progress rather than circling obstacles indefinitely
\end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{img/Hard_cnn_mlp.png}
